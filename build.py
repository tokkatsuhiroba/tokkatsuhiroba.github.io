...
SETSU_NA = {
    ...
    # 2026-09-23 依頼：学習過程と一次資料を「明日から使えるグッズ」から
    #   「特活とは」へ移しました。グッズを探しに来た人に、学習過程が
    #   先に出るのは順序が逆だったためです。
    'katei':  '学習過程',   'ichiji': '一次資料',
...
def kenmon_jissen(fm, goods):
    ...
    #
    # ★指した先が無いとき、**ここは止めます**（2026-09-24 に そう決めました）。
    #   答え（load_kotae）は同じ場合に「絵だけ落として、字は残す」です。
    #   決まりが2つに分かれているのは、うっかりではありません。理由はこうです。
    #     ・実践の札は、**写真そのものが中身**です。抜けると札に何も残りません。
    #       答えの本体は字で、写真は「あれば助かるもの」です。無くても答えです。
    #     ・答えは1つのお悩みに、何人ぶら下がるか分かりません。そのうち1件の
    #       写真が消えていただけで、**ほかの人の答えまで巻きこんで**全部が
    #       止まるのは割に合いません。実践は1件が1枚の札で、そこは独立です。
    #     ・ゆるめると、送りなおした先生の目には「通った」と見えたまま、
    #       **写真だけが黙って消えた実践**が公開されます。止めれば、直るまで
    #       前の形が出たままになります。出さないより、古いほうがましです。
    #   ここをゆるめるときは、上の3つが変わったのかを先に確かめてください。
    #   （2026-09-23 夜に、ここで実際にサイト全体が2回止まりました。原因は
    #     この検問ではなく、受け口が「古い写真を消してから、新しい写真を
    #     置かずに帰った」ことでした。入り口のほうを直してあります。）
...
def load_kotae():
    ...
    for p in sorted(glob.glob(os.path.join(KOTAE, '*.md'))):
        ...
        #   ★実践（kenmon_jissen の bansho:）は、同じ場合に **止めます**。
        #     わざと分けています。なぜ2つなのかは、あちらに書きました。
        #     片方を触るときは、もう片方も読んでください（2026-09-24）。
...
KOTOBA = {
    ...
    # ── 学習過程・一次資料を「特活とは」へ移した ぶん（2026-09-23）──
    ...
    '学級活動(1)が、どんな順で回るか。①から⑤がひと回りして、また①に戻ります。':
        ('The order Classroom Activities (1) go round in. ① to ⑤ make one turn, '
         'then it is back to ①.',
         'الترتيب الذي تدور به أنشطة الفصل (١). من ① إلى ⑤ دورة كاملة، ثم العودة إلى ①.'),
    '根拠になるものへ、直接つなぎます。PDFや外部ページは、新しいタブで開きます。':
        ('Links straight to what it is based on. PDFs and external pages open in a new tab.',
         'روابط مباشرة إلى ما يستند إليه. وتُفتح ملفات PDF والصفحات الخارجية في تبويب جديد.'),
    '特別活動には、教科書がありません。<br>グッズがあると、子どもたちが自分たちで進められます。':
        ('Special Activities have no textbook.<br>'
